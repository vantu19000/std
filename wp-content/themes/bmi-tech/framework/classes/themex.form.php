...
class ThemexForm {
		...
		if(get_post_type()=='tour') {			
			...
			if($slug!='search') {
				$out.=ThemexInterface::renderOption(array(
					...
					'name' => __('Form Email', 'quangbinhtravel'),
					'type' => 'text',
					'description' => __('Enter email address to use instead of the default email address', 'quangbinhtravel'),
					'value' => isset(self::$data[$slug]['email'])?self::$data[$slug]['email']:'',
				));
				
				$out.=ThemexInterface::renderOption(array(
					'id' => __CLASS__.'['.$slug.'][description]',
					'name' => __('Form Description', 'quangbinhtravel'),
					'type' => 'textarea',
					'description' => __('Enter form description to show before the form fields', 'quangbinhtravel'),
					'value' => isset(self::$data[$slug]['description'])?themex_stripslashes(self::$data[$slug]['description']):'',
				));
				
				$out.=ThemexInterface::renderOption(array(
					'id' => __CLASS__.'['.$slug.'][message]',
					'name' => __('Form Message', 'quangbinhtravel'),
					'type' => 'textarea',
					'description' => __('Enter form message to show when form is submitted', 'quangbinhtravel'),
					'value' => isset(self::$data[$slug]['message'])?themex_stripslashes(self::$data[$slug]['message']):'',
				));
				
				$out.=ThemexInterface::renderOption(array(
					'id' => __CLASS__.'['.$slug.'][captcha]',
					'name' => __('Enable Captcha', 'quangbinhtravel'),
					'type' => 'checkbox',
			...
			$out.=ThemexInterface::renderOption(array(
				'name' => __('Form Fields', 'quangbinhtravel'),
				'type' => 'title',
			));
	...
	public static function renderData($slug, $format,  $values=array()) {
		...
		if(self::isActive($slug)) {
			...
			if(isset(self::$data[$slug]['captcha'])) {
				...
				$out.='<input type="text" name="captcha" id="captcha" size="6" value="" placeholder="'.__('Code', 'quangbinhtravel').'" /></div>';
			}
		}
	...
	public static function renderField($field) {
		...
		$out.='<a class="themex-button themex-remove-button themex-trigger" title="'.__('Remove', 'quangbinhtravel').'" data-action="themex_form_remove" data-element="'.$field['form'].'_'.$field['id'].'"></a>';
		...
		$out.=ThemexInterface::renderOption(array(
			...
			'value' => $field['form'],
			'wrap' => false,
			'after' => '<a class="themex-button themex-add-button themex-trigger" title="'.__('Add', 'quangbinhtravel').'" data-action="themex_form_add" data-element="'.$field['form'].'_'.$field['id'].'" data-value="'.$field['form'].'_'.$field['id'].'_value"></a>',				
		));
		
		$out.=ThemexInterface::renderOption(array(
			'id' => __CLASS__.'['.$field['form'].'][fields]['.$field['id'].'][name]',
			'type' => 'text',
			'attributes' => array('placeholder' => __('Name', 'quangbinhtravel')),
		...
		if($field['form']=='search') {
			$out.=ThemexInterface::renderOption(array(
				...
				'type' => 'select',
				'options' => array(
					'text' => __('String', 'quangbinhtravel'),
					'date' => __('Date', 'quangbinhtravel'),
					'select' => __('Select', 'quangbinhtravel'),			
				),
			...
			$out.=ThemexInterface::renderOption(array(
				...
				'options' => array(
					'text' => __('String', 'quangbinhtravel'),
					'number' => __('Number', 'quangbinhtravel'),
					'email' => __('Email', 'quangbinhtravel'),
					'date' => __('Date', 'quangbinhtravel'),
					'textarea' => __('Text', 'quangbinhtravel'),					
					'select' => __('Select', 'quangbinhtravel'),
					'checkbox' => __('Checkbox', 'quangbinhtravel'),
				),
				'value' => isset(self::$data[$field['form']]['fields'][$field['id']]['type'])?self::$data[$field['form']]['fields'][$field['id']]['type']:'',
				'wrap' => false,
			));
		...
		$out.=ThemexInterface::renderOption(array(
			...
			'type' => 'text',
			'attributes' => array('placeholder' => __('Options', 'quangbinhtravel')),
		...
		if($field['form']!='search') {
			$out.=ThemexInterface::renderOption(array(
				...
				'name' => __('Optional', 'quangbinhtravel'),
				'type' => 'checkbox',
		...
		if(isset($data['slug']) && self::isActive($data['slug'])) {
			if(isset(self::$data[$data['slug']]['captcha'])) {
				...
				if($session_code!= $posted_code) {
					ThemexInterface::$messages[]=__('The verification code is incorrect', 'quangbinhtravel');
				}
			}
			...
			foreach(self::$data[$data['slug']]['fields'] as $field) {
				...
				if(((!isset($data[$ID]) || trim($data[$ID])=='') || ($field['type']=='select' && $data[$ID]==0)) && $field['type']!='checkbox' && !isset($field['optional'])) {
					ThemexInterface::$messages[]='"'.$field['name'].'" '.__('field is required', 'quangbinhtravel');
				} else {
					if($field['type']=='number' && !is_numeric($data[$ID])) {
						ThemexInterface::$messages[]='"'.$field['name'].'" '.__('field can only contain numbers', 'quangbinhtravel');
					}
					
					if($field['type']=='email' && !is_email($data[$ID])) {
						ThemexInterface::$messages[]=__('You have entered an invalid email address', 'quangbinhtravel');
					}
				}
			}
			...
			if(!empty(ThemexInterface::$messages)) {
				...
				$subject=__('Contact', 'quangbinhtravel');
				if($data['slug']=='booking') {
					$subject=__('Booking', 'quangbinhtravel');
				} else if($data['slug']=='question') {
					$subject=__('Question', 'quangbinhtravel');
				}
				
				$message='';
				if(in_array($data['slug'], array('booking', 'question'))) {
					$message.=__('Tour', 'quangbinhtravel').': '.get_the_title($data['id']).'<br />';
				}
				...
				foreach(self::$data[$data['slug']]['fields'] as $field) {
					...
					if($field['type']=='checkbox') {
						if(isset($data[$ID])) {
							$data[$ID]=__('Yes', 'quangbinhtravel');
						} else {
							$data[$ID]=__('No', 'quangbinhtravel');
						}
					} else if($field['type']=='select') {
