...
class ThemexInterface {
	...
	public static function renderTBUploader($fields, $post) {
		...
		//send button
		$send_button="<input type='submit' class='button' name='send[$attachment_id]' value='".__( 'Insert This Item' , 'quangbinhtravel' )."' />&nbsp;&nbsp;&nbsp;";
	...
	public static function addPage() {
		add_theme_page(__('Theme Options','quangbinhtravel'), __('Theme Options','quangbinhtravel'), 'administrator', 'theme-options', array(__CLASS__,'renderPage'));
	}
	...
	public static function renderOption($option) {
		...
		switch($option['type']) {
			...
			case 'uploader':
				...
				$out.='<a class="button themex-upload-button">'.__('Browse','quangbinhtravel').'</a>';
			break;
			...
			case 'select_post':				
				...
				$out.='<select id="'.$option['id'].'" name="'.$option['id'].'" '.$attributes.'>';
				$out.='<option value="0">'.__('None', 'quangbinhtravel').'</option>';
			...
			case 'select_category':			
				$args=array(
					...
					'echo'=> 0,
					'selected' => $option['value'],
					'show_option_all' => __('None', 'quangbinhtravel'),
					'hierarchical' => 0, 
					'name' => $option['id'],
					'id' => $option['id'],
					'depth' => 0,
	...
	public static function renderCommentForm($fields) {
		...
		$fields['fields']['author']='<div class="cm-column"><div class="field-container"><input id="author" name="author" type="text" value="" size="30" placeholder="'.__('Name', 'quangbinhtravel').'" /></div></div>';
		$fields['fields']['email']='<div class="cm-column last"><div class="field-container"><input id="email" name="email" type="text" value="" size="30" placeholder="'.__('Email', 'quangbinhtravel').'" /></div></div>';
		$fields['fields']['url']='';
		$fields['comment_field']='<div class="field-container"><textarea id="comment" name="comment" cols="45" rows="8" placeholder="'.__('Comment', 'quangbinhtravel').'"></textarea></div>';
		
		$fields['title_reply']='';
		$fields['title_reply_to']='';
		$fields['label_submit']=__('Add Comment', 'quangbinhtravel');
	...
	public static function renderPageTitle() {
		...
		if(is_single()) {
			...
			$out=__('Profile', 'quangbinhtravel');
		}

		if(empty($out)) {
			$out=__('Archives', 'quangbinhtravel');
		}
