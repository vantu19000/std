...
class ThemexSubscribe extends WP_Widget {
	...
	function __construct() {
		$widget_ops=array('classname' => 'widget-subscribe', 'description' => __('Mailing list signup form', 'quangbinhtravel'));
		parent::__construct('widget-subscribe', __('Newsletter', 'quangbinhtravel'), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		$instance=wp_parse_args((array)$instance, array(
			'title' => __('Newsletter', 'quangbinhtravel'),
			'description' => '',
		));
		
		$title=apply_filters('widget_title', empty($instance['title'])?__('Newsletter', 'quangbinhtravel'):$instance['title'], $instance, $this->id_base);
		...
		$out.='<div class="field-container"><input type="text" name="email" placeholder="'.__('Email Address', 'quangbinhtravel').'" /></div>';
	...
	function submitData() {
		...
		if(!isset($data['email']) || empty($data['email'])) {
			ThemexInterface::$messages[]=__('Email address is required', 'quangbinhtravel');
			ThemexInterface::renderMessages();
		} else if(!is_email($data['email'])) {
			ThemexInterface::$messages[]=__('Email address is incorrect', 'quangbinhtravel');
			ThemexInterface::renderMessages();
		} else {
			...
			if(!in_array($data['email'], $emails)) {
				...
				ThemexInterface::$messages[]=__('Thank you for subscribing!', 'quangbinhtravel');
				ThemexInterface::renderMessages(true);
			} else {
				ThemexInterface::$messages[]=__('You have already subscribed', 'quangbinhtravel');
				ThemexInterface::renderMessages();
			}
		}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('description'); ?>"><?php _e('Description', 'quangbinhtravel'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('emails'); ?>"><?php _e('Mailing List', 'quangbinhtravel'); ?>:</label>
