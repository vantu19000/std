...
function themex_add_string($key, $type, $string) {
	...
	if(!empty($string) && !isset($strings[$name])) {
		...
		if($file!==false) {
			fwrite($file, "\r\n".'$strings'."['".$name."']=__('".$string."', 'quangbinhtravel');");
			fclose($file);
		}
	}
}
...
add_action('restrict_manage_posts','tour_filters');
function tour_filters(){
    global $typenow;
    $select="";
    if( $typenow == 'tour' ){
        $option=array("0"=>"Tour default","1"=>"Tour Hot");
        echo "<select name='tour_hot'>";
		echo "<option value=''>All Tour</option>";
        foreach ($option as $k=>$v){
            if(isset($_GET['tour_hot']))
                $selected= $k==$_GET['tour_hot']?'selected':'';
...
add_filter( 'parse_query','tour_fillters_query');
function tour_fillters_query( $query )
{
    global $typenow;
    global $pagenow;


    if( $pagenow == 'edit.php' && $typenow == 'tour' && $_GET['tour_hot'] )
    {
        $query->query_vars['meta_key'] = '_'.THEMEX_PREFIX.'hot';
        $query->query_vars[ 'meta_value' ] = (int)$_GET['tour_hot'];
    }
}
function getRatingComment($post_id){
	$arr=array();
	$arr["avg_rating"]=get_post_meta($post_id, 'crfp-average-rating', true);
	$count_comment=wp_count_comments( $post_id );
	$arr["count_comment"]=$count_comment->total_comments;
	return $arr;
}
/****************** custom column **********************/

function tour_columns_head($defaults) {
    $defaults['tour_hot'] = 'Hot';
    return $defaults;
}
 
// SHOW THE FEATURED IMAGE
function tour_columns_content($column_name, $post_ID) {
    if ($column_name == 'tour_hot') {
       $hot=get_post_meta($post_ID, '_'.THEMEX_PREFIX.'hot', true);
       if ($hot==1) {
          echo '<span class="y_hot"></span>';
       }else{
		  echo '<span class="n_hot"></span>';
	   }
    }
}
add_filter('manage_posts_columns', 'tour_columns_head');
add_action('manage_posts_custom_column', 'tour_columns_content', 10, 2);